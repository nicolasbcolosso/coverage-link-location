...
### Policy**c and Endorsement**c (Screen Actions)
...
**Recommendation**: Before enabling `Is_New_Property__c`, ensure:

1. Account has Location records created
2. Existing Property coverages without Location are either migrated or no longer needed
...
| `CoverageSelectionControllerTest.cls`       | Apex Test    | Test coverage                   |
| `coverageSelection`                         | LWC          | New coverage selection UI       |
| `coverageSelectionRouter`                   | LWC          | Routes to old/new component     |
| `Policy__c.Add_Remove_Coverage_Router`      | Quick Action | Screen action for Policy        |
| `Endorsement__c.Add_Remove_Coverage_Router` | Quick Action | Screen action for Endorsement   |
