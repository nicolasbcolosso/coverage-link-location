...
export default class Lookup extends LightningElement {
    ...
    openSearchModal(){
        console.log('----------------------> cheese');
        const openSearchEvent = new CustomEvent('opensearch', {
            detail: true
        });
        
        this.dispatchEvent(openSearchEvent);
    }
    openSearchModalAcc(){
        console.log('----------------------> cheese');
        const openSearchEvent = new CustomEvent('opensearchacc', {
            detail: true
        });
    ...
    setSearchResultsAcc(results) {
        ...
        // Clone results before modifying them to avoid Locker restriction
        const resultsLocal = JSON.parse(JSON.stringify(results));
        //this.searchResults = resultsLocal;
        // Format results
        this.searchResults = resultsLocal.map((result) => {
            // Clone and complete search result if icon is missing
            if (this.searchTerm.length > 0) {
    ...
    connectedCallback(){
        if (this.apinameField == 'Secondary_Marketer_UW_Name__c') {
    ...
    handleResultClick(event) {
        ...
        const selectsucces = new CustomEvent('selectionchange', {
            detail: this.curSelection
        });
    ...
    get getSearchIconClass() {
        let css = ' slds-input__icon_right ';
        if (!this.isMultiEntry) {
